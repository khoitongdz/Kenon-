...
local discordInvites = {
    {Name = "ᴋʜᴏɪᴛᴏɴɢᴅᴢ | ᴄᴏᴍᴍᴜɴɪᴛʏ", Invite = "https://discord.gg/2NpBPjM4PF", Logo = "rbxassetid://105486552530887"},
}
...
-- Create game tabs
local tabs = {
    C = Window:MakeTab({"Script phụ", "file"}),
    A = Window:MakeTab({"Blox Fruit", "apple"}),
    B = Window:MakeTab({"Dead Rails", "train"}),
}
...
-- Attack Aura Toggle (tabs.C)
tabs.C:AddToggle({
    Name = "Attack Aura",
    Description = "Tự động tấn công kẻ địch gần",
    Default = _G.Settings.Setting["Attack Aura"],
...
-- BringMob Mode Dropdown
SettingsTab:Dropdown("BringMob Mode", { "Low", "Normal", "High" }, _G.Settings.Setting["Bring Mob Mode"], function(value)
    _G.Settings.Setting["Bring Mob Mode"] = value
    getgenv().SaveSetting()
end)

-- Tự động cập nhật khoảng cách BringMob
spawn(function()
    while task.wait() do
        pcall(function()
            local mode = _G.Settings.Setting["Bring Mob Mode"]
            if mode == "Low" then
...
-- Gán độ trễ tấn công tương ứng với Fast Attack Mode
spawn(function()
    while task.wait() do
...
-- Tự động tấn công khi Attack Aura bật
spawn(function()
